...
public class ProfilePicture : MonoBehaviour
{
	...
	public void SetPicture(Sprite pic, int rotationCode = 0, float scale = 1f)
    {
		...
		switch (rotationCode)
		{
			...
			case 5: rotationLevel = 3; hFlip = true; break;
			case 6: rotationLevel = 1; break;
			case 7: rotationLevel = 1; hFlip = true; break;
			case 8: rotationLevel = 3; break;
			default: break;
		}
		sr.sprite = pic;
		sr.transform.localScale = new Vector3(1f * (hFlip ? -1f : 1f), 1f, 1f) * scale * (isSquare ? 1.5f : isRound ? .85f : 1f);
